#!/bin/sh
# Install the Provider Watcher add-on for the Hallow provider.
#
...
REPO_OWNER="Maximillian-Detwiller"
REPO_NAME="music-assistant-hallow-provider"
ADDON_SLUG="hallow_provider_watcher"
ADDON_NAME="Hallow Provider Watcher"
...
for _candidate in "$TMPDIR"/*/; do
    if [ -d "${_candidate}hallow" ]; then
        SRC_ROOT="${_candidate%/}"
        break
    fi
done
if [ -z "$SRC_ROOT" ] || [ ! -d "$SRC_ROOT/hallow" ]; then
    die "hallow/ not found in the archive downloaded from $TARBALL_URL"
fi
...
mkdir -p "$ADDON_DIR"
cp -R "$SRC_ROOT/hallow" "$ADDON_DIR/hallow"
...
name: "$ADDON_NAME"
description: "Re-installs the Hallow provider into Music Assistant after every container restart. Bundles provider $PROVIDER_VERSION (from $REF)."
...
configuration:
  auto_update:
    name: Keep the Hallow provider up to date
    description: >-
      Off by default. When enabled, periodically check GitHub for a newer
      Hallow provider and reinstall it (restarting the server) only when the
      code actually changed. Note this downloads and runs branch-head code
      inside the server unattended. This is NOT the add-on's own "Auto update"
      control on the Info tab, which updates the watcher add-on itself; this
      option updates the music provider inside the server.
...
fetch_latest() {
    ...
    nd="$(find "$tmp" -maxdepth 3 -type d -name hallow 2>/dev/null | head -n1)"
    if [ -z "$nd" ]; then
        echo "auto-update: hallow not found in tarball"; rm -rf "$tmp"; return 1
    fi
...
MA="$MA_ID"
BUNDLED="/provider/hallow"
CACHE="/data/hallow"
HASHFILE="/data/hallow.sha256"
...
install_provider() {
    src="\$(provider_src)"
    echo "[\$(date)] Installing Hallow provider from \$src ..."
    sleep 3
    ...
    docker exec "\$MA" rm -rf "\$DST/hallow" 2>/dev/null || true
    ...
    if [ "\$AUTO_UPDATE" = "true" ]; then
        ...
        if [ \$((now - LAST_UPDATE)) -ge "\$UPDATE_INTERVAL" ]; then
            ...
            if fetch_latest; then
                ...
                log "auto-update: Hallow provider \$FETCHED_VERSION is new here -> reinstalling"
                CUR_ID=\$(docker ps -q --no-trunc --filter name="\$MA" 2>/dev/null)
